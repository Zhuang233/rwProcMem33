...
static long hwBreakpointProc_ioctl(
	...
	case IOCTL_CLOSE_HANDLE: //关闭进程，减少引用计数
	{
		/*
		驱动通信协议：
	...
	case IOCTL_ADD_PROCESS_HWBP: //设置进程硬件断点
	{
		...
		*/
		...
		attr.disabled = 0;// 创建后立即开始计数，不用显式开启计数
		sample_hbp = register_user_hw_breakpoint(&attr, sample_hbp_handler, NULL, task); //sample_hbp_handler是断点回调函数
